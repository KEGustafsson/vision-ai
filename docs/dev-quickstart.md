...
- Vision container: http://localhost:7000/stream/forward.mjpg
- SignalK server: http://localhost:3000 (enable the **Marine Vision-AI** plugin
  under *Server → Plugin Config*, set `containerUrl` to `http://vision-service:7000`)
...
curl "localhost:7000/events/recent?n=1" | jq '.[0].targets[].label'
# open localhost:7000/stream/aft.mjpg  → note the red "MOB!" box (person in water)
```
