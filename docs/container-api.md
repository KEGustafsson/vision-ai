...
```jsonc
{
  ...
  "optical_flow": {                // NVIDIA OFA per camera; {} unless enabled (DeepStream)
    "forward": {
      "enabled": true,
...
`optical_flow` is a measurement only — it feeds no detection, geometry or alert
logic, and stale/absent flow does not degrade `status` unless
